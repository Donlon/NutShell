...
trait HasNutCoreParameter {
  ...
  val VAddrBits: Int = Settings.getValue("VAddrBits", if (Settings.get("IsRV32")) 32 else 39) // VAddrBits is Virtual Memory addr bits
  ...
  val MModeOnly = Settings.get("MmodeOnly")
}
