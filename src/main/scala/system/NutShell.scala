...
class NutShell(implicit val p: NutCoreConfig) extends Module with HasSoCParameter {
  ...
  val mem = if (HasL2cache) {
    ...
    l2cacheOut <> Cache(in = l2cacheIn.retiming(respForwardPath = true),
      mmio = 0.U.asTypeOf(new SimpleBusUC) :: Nil, flush = "b00".U, empty = l2Empty, enable = true)(
  ...
  val addrSpace : List[List[(Long, Long)]] = List(
    List((0xd8000000L, 0x00010000L)), // CLINT
    List((0xdc000000L, 0x04000000L)), // PLIC
    List((Settings.getLong("MMIOBase"), Settings.getLong("MMIOSize"))), // external devices and defaults
  )
  ...
  clint.io.in <> mmioXbar.io.out(0).retiming(reqForwardPath = true).toAXI4Lite("clint_axi4lite_bridge")
